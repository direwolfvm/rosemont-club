...
Enabled on tenant `alex311-qfnem` on September 22, 2026 (`defaultSupportedIdpConfigs/apple.com`, client ID `com.rosemont.rosemontclub`). The app exchanges Apple's identity token through `accounts:signInWithIdp` with `providerId=apple.com`; the token audience is the App Store bundle ID, which is a registered Firebase iOS app. No Services ID is configured because the website does not offer Apple sign-in. The provider's `bundleIds` list both bundles. Apple requires apps to revoke Sign in with Apple tokens when an account is deleted; Firebase does that when the provider has an Apple private key (`scripts/apple-signin-key.py` applies one from a downloaded `.p8` file) and the app passes Apple's authorization code to `accounts:revokeToken` during deletion. Neighbors who choose Hide My Email get an `@privaterelay.appleid.com` address; for Mailgun mail to reach them, `mg.rosemont.club` and the from-address must be registered under Sign in with Apple for Email Communication in the Apple Developer account (a portal task).
